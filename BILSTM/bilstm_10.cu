...
#include <ctime>
#include <sys/time.h>
#include "readWeights10.h"//to read the weights
#include "deviceFunctions10.h"//contains device functions like matmul,add
using namespace std;
...
__global__ void predictKernel(double *X,double *W_i,double *W_f,double *W_c,double *W_o,double *U_i,double *U_f,double *U_c,double *U_o,double *b_i,double *b_f,double *b_c,double *b_o,double *w,double *b,double *result,double *loop_count)//cuda kernel
	{
		...
		//printf("Done");
		loop_count[0]=0;
		double x[10][3];//input to lstm,50 timestamps
		double *c_t,*h_t,*i_t,*C_t,*f_t,*o_t;
		double H[10][20];//storing the output of each timestamp(50 timestamps, each output of size 50)
		double input[20],output[12];//input & output of dense layer
		double pd1[12],pd2[12];//probabbility density for upper and lower window resp.
		int i,j;
		double sum,res;
		if ((tid>9&&tid<1719551-10))
		{
			//create upper window
			#pragma unroll
			for(i=9;i>=0;i--)//i :timestamp from 0-49
			{
				x[i][0]=*(X+(tid-(9-i))*3+0);
				x[i][1]=*(X+(tid-(9-i))*3+1);
				x[i][2]=*(X+(tid-(9-i))*3+2);
			...
			#pragma unroll
			 for(i=9;i>=0;i--)//i :timestamp from 0-49
                	{
                        x[9-i][0]=*(X+(tid-(9-i))*3+0);
                        x[9-i][1]=*(X+(tid-(9-i))*3+1);
                        x[9-i][2]=*(X+(tid-(9-i))*3+2);
			loop_count[0]++;
                	}
			#pragma unroll
			for(i=0;i<10;i++)//i: timestamp(t)
			...
			//create lower window
			#pragma unroll
			for(i=0;i<10;i++)//i :timestamp from 0-49
			{
				x[i][0]=*(X+(tid+i)*3+0);
				x[i][1]=*(X+(tid+i)*3+1);
				x[i][2]=*(X+(tid+i)*3+2);
...
int main()
{
	...
	double *W_i=(double *)malloc(10*3*sizeof(double));
	double *W_f=(double *)malloc(10*3*sizeof(double));
	double *W_c=(double *)malloc(10*3*sizeof(double));
	double *W_o=(double *)malloc(10*3*sizeof(double));
	double *U_i=(double *)malloc(10*10*sizeof(double));
	double *U_f=(double *)malloc(10*10*sizeof(double));
	double *U_c=(double *)malloc(10*10*sizeof(double));
	double *U_o=(double *)malloc(10*10*sizeof(double));
	double *b_i=(double *)malloc(10*sizeof(double));
	double *b_f=(double *)malloc(10*sizeof(double));
	double *b_c=(double *)malloc(10*sizeof(double));
	double *b_o=(double *)malloc(10*sizeof(double));
	double *w=(double *)malloc(20*12*sizeof(double));
	...
	size_t bytes2=10*3*sizeof(double);
	size_t bytes3=10*10*sizeof(double);
	size_t bytes4=10*sizeof(double);
	size_t bytes5=20*12*sizeof(double);
