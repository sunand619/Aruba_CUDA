...
#include <ctime>
#include <sys/time.h>
#include "readWeights50.h"//to read the weights
#include "deviceFunctions50.h"//contains device functions like matmul,add
using namespace std;
...
__global__ void predictKernel(double *X,double *W_i,double *W_f,double *W_c,double *W_o,double *U_i,double *U_f,double *U_c,double *U_o,double *b_i,double *b_f,double *b_c,double *b_o,double *w,double *b,double *result,double *loop_count)//cuda kernel
	{
		...
		if ((tid>49&&tid<1719551-50))
		{
			...
			#pragma unroll
			 for(i=49;i>=0;i--)//i :timestamp from 0-49
                	{
                        x[49-i][0]=*(X+(tid-(49-i))*3+0);
                        x[49-i][1]=*(X+(tid-(49-i))*3+1);
                        x[49-i][2]=*(X+(tid-(49-i))*3+2);
			loop_count[0]++;
                	}
			#pragma unroll
			for(i=0;i<50;i++)//i: timestamp(t)
		...
		#pragma unroll
                for(i=0;i<50;i++)//i :timestamp from 0-49
                {
                        x[49-i][0]=*(X+(tid+i)*3+0);
                        x[49-i][1]=*(X+(tid+i)*3+1);
                        x[49-i][2]=*(X+(tid+i)*3+2);
			loop_count[0]++;
                }
                //prediction  for lower window
		#pragma unroll
...
int main()
{
	...
	size_t bytes5=100*12*sizeof(double);
