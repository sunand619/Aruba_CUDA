...
#include <ctime>
#include <sys/time.h>
#include "readWeights.h"//to read the weights
#include "deviceFunctions.h"//contains device functions like matmul,add
using namespace std;
...
__global__ void predictKernel(double *X,double *W_i,double *W_f,double *W_c,double *W_o,double *U_i,double *U_f,double *U_c,double *U_o,double *b_i,double *b_f,double *b_c,double *b_o,double *w,double *b,double *result)//cuda kernel
	{
		...
		double H[50][50];//storing the output of each timestamp(50 timestamps, each output of size 50)
		double input[50],output[12];//input & output of dense layer
		double pd1[12],pd2[12];//probabbility density for upper and lower window resp.
		int i,j;
		double sum,res;
		if ((tid>49&&tid<1719551-50))
		{
			//create upper window
			//#pragma unroll
			for(i=49;i>=0;i--)//i :timestamp from 0-49
			{
				x[i][0]=*(X+(tid-(49-i))*3+0);
				x[i][1]=*(X+(tid-(49-i))*3+1);
				x[i][2]=*(X+(tid-(49-i))*3+2);
			}
...
int main()
{
	...
	double *w=(double *)malloc(50*12*sizeof(double));
	...
	//printf("%f ",*(b_i+p));
	//printf("\n");
	double *X_gpu,*W_i_gpu,*W_f_gpu,*W_c_gpu,*W_o_gpu,*U_i_gpu,*U_f_gpu,*U_c_gpu,*U_o_gpu,*b_i_gpu,*b_f_gpu,*b_c_gpu,*b_o_gpu,*w_gpu,*b_gpu,*result_gpu;//device vector
	
	size_t bytes1=1719551*3*sizeof(double);//size in bytes of the vector to be sent to gpu
	size_t bytes2=50*3*sizeof(double);
	size_t bytes3=50*50*sizeof(double);
	size_t bytes4=50*sizeof(double);
	size_t bytes5=50*12*sizeof(double);
	...
	totalThreads=1719551;
	gettimeofday(&t, NULL);
	fs_t = t.tv_sec+(t.tv_usec/1000000.0);
	cudaEventRecord(start);
	predictKernel<<<gridSize, blockSize>>>(X_gpu,W_i_gpu,W_f_gpu,W_c_gpu,W_o_gpu,U_i_gpu,U_f_gpu,U_c_gpu,U_o_gpu,b_i_gpu,b_f_gpu,b_c_gpu,b_o_gpu,w_gpu,b_gpu,result_gpu);
	...
	ft_t = fe_t - fs_t;
	cout<<loop_count<<' '<<smCount<<' '<<cudaCores<<' '<<totalThreads<<'\n';
